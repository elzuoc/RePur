    ...
    <div class="relative w-full mt-4 text-left">
      ...
      >
        <div
          ...
          @click="
            sortProductList({ event: $event, field: 'buy_date', type: 'desc' })
          "
        >
          <img
            src="@/assets/images/icons/sort-desc.svg"
        ...
        <div
          class="p-2 hover:bg-zinc-50 cursor-pointer"
          @click="
            sortProductList({ event: $event, field: 'buy_date', type: 'asc' })
          "
        >
          <img
      ...
      <div class="p-2 hover:bg-zinc-50 cursor-pointer" @click="setSortOption($event)">
        <img src="@/assets/images/icons/view-list.svg" class="w-5 inline-block" />
        年度
      </div>
      <div class="p-2 hover:bg-zinc-50 cursor-pointer" @click="setSortOption($event)">
        <img src="@/assets/images/icons/star.svg" class="w-5 inline-block" />
        品牌
      </div> -->
      </div>
    </div>
...
const apiSortProductList = (params) => axios.get(
  `/api/GET/products?sort=true&sortField=${params.field}&sortType=${params.type}`,
);
  ...
  try {
    ...
    products.value = data.map((item) => {
      const parseText = (value) => {
        const findChannel = channelOptions.value.find(
...
const newImg = (src) => new Promise((resolve, reject) => {
...
const filterSearchOptions = (e) => {
  searchOptions.value = Array.from(products.value).filter((item) => item.name.includes(e.target.innerHTML));
...
const goSearch = async () => {
  const params = {
    prodName:
      search.value.textContent.trim() === 'Search...'
        ? null
        : search.value.textContent.trim(),
  };
